...
public unsafe partial class FhBattleAPIs {
    ...
    private void DmgCalc_InflictStatus(DmgEventArgs e) {
        const byte INFINITE = 255;
        const byte ALWAYS = 254;
        ...
        for (int status_idx = 0; status_idx <= 0x18; status_idx++) {
            ...
            if (e.command->is_cleanse) {
                ...
                if (temporal_idx < 0 && permanent_mask.threaten()) {
                    ...
                    if (infliction2 == INFINITE) {
                        status_hit = true;
                    } else {
                        resistance = resistance2;
                        if (resistance != INFINITE) {
                            if (infliction2 == ALWAYS) status_hit = true;
                            if (rng % 101 < infliction2 - resistance) status_hit = true;
                            if (Globals.Battle.btl->debug.always_hit) status_hit = true;
                        }
                    }
                }
            }
    ...
    private void DmgCalc_InflictStatus_Permanent(
        ...
        if (permanent_mask.threaten()) {
            bool has_auto_haste_or_slow = e.target_status_suffer_duration.haste == INFINITE
                                       || e.target_status_suffer_duration.slow == INFINITE;
    ...
    private void DmgCalc_InflictStatus_Temporary(
        ...
        if (e.command->is_cleanse) {
            ...
            if (turns_left is 0 or INFINITE) {
                e.status_misses += 1;
                return;
            }
        ...
        bool immune_to_haste_slow = e.target_status_suffer_duration.haste == INFINITE
                                 || e.target_status_suffer_duration.slow == INFINITE
